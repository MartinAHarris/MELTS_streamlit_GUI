...
def relative_viscosity_mueller(phi_c, phi_m):
    ...
    Modified to allow φc > φm and return very large viscosities instead of ∞.
    """
    if phi_m is None or phi_m <= 0:
        return None

    # Standard Mueller region
    if phi_c < phi_m:
        return (1.0 - phi_c / phi_m) ** -2

    # Extended jammed regime: smooth continuation
    excess = phi_c - phi_m
    # Large but finite growth beyond φm
    return 1e6 * (1 + 50 * excess)**2
...
st.header("Bulk Composition (wt%)")#change this to match your own desired value or edit in the GUI #this is AW18_022

SiO2 = st.number_input("SiO2", value=49.74)
TiO2 = st.number_input("TiO2", value=1.6)
Al2O3 = st.number_input("Al2O3", value=15.77)
FeOtot = st.number_input("FeO*", value=11.54)
MnO = st.number_input("MnO", value=0.17)
MgO = st.number_input("MgO", value=7.74)
CaO = st.number_input("CaO", value=8.83)
Na2O = st.number_input("Na2O", value=3.64)
K2O = st.number_input("K2O", value=0.49)
P2O5 = st.number_input("P2O5", value=0.28)
...
if mode == "Sub-liquidus Equilibrium":
    ...
    if relvisc_on:
        ...
        if liq_source == "User VFT parameters":
            A_user = st.number_input("A (dimensionless)", value=-2.38)
            B_user = st.number_input("B (K)", value=2717.19)
            C_user = st.number_input("C (K)", value=742.15)
...
if run:
    ...
    if 'relvisc_on' in locals() and relvisc_on:
        ...
        # 5. Relative viscosity and suspension viscosity (always finite by construction)
        ...
        st.write(f"log₁₀ η_rel: {math.log10(eta_rel):.3f}")
        ...
        # 7. Plot: log₁₀ η_rel vs φc (extended beyond φm)
        st.subheader("Relative Viscosity Curve (Mueller Eq. 3, extended)")
        if phi_m_mix is not None and phi_m_mix > 0:
            # up to 1.5 * φm to see jammed regime
            phi_vals = [i / 100.0 * (phi_m_mix * 1.5) for i in range(0, 151)]
            eta_rel_vals = [relative_viscosity_mueller(phi, phi_m_mix) for phi in phi_vals]
            df_rel = pd.DataFrame({
                "phi_c": phi_vals,
                "log10_eta_rel": [
                    math.log10(v) if (v is not None and v > 0) else None
                    for v in eta_rel_vals
                ]
            })
            st.line_chart(df_rel.set_index("phi_c")["log10_eta_rel"])
            st.caption("log₁₀(η_rel) vs φc, including jammed regime (φc > φm).")
